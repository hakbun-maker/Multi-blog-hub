...
```
STEP 0. 입력 정렬 — Intent 반영 ★
  배분 우선순위 = KeywordGrade 순위 × Intent Priority Score(IPS)
  IPS: AD=1.0, COMPARE=0.9, REVIEW=0.7, CRITIC=0.5, INFO=0.4, TREND=0.2
  → 같은 S급 키워드라도 AD형이 TREND형보다 먼저 S급 블로그에 배정
...
STEP 5. Intent 적합도 적용 (2차 필터) ★
  Intent × BlogGrade 적합도 매트릭스로 후보 재정렬:
            S급     A급     B급     NEW
  AD        3최적   2양호   1가능   0부적합
  COMPARE   3최적   2양호   1가능   0부적합
  REVIEW    2양호   3최적   1가능   0부적합
  CRITIC    1가능   3최적   2양호   0부적합
  INFO      1가능   2양호   3최적   1가능
  TREND     0부적합 1가능   3최적   2양호

  적합도 0(부적합) → 후보군에서 제거
  예: TREND 키워드 → S급 블로그 후보에서 제거 (저CPC로 카테고리 일관성 훼손 방지)
  예: AD 키워드 → NEW 블로그 후보에서 제거 (신뢰도 부족으로 전환율 낮음)
...
STEP 8. 최종 선택
  남은 후보 중 Intent 적합도 내림차순 → 동점이면 blog_score 내림차순 → 1위 선택

STEP 9. 배정 및 기록
  cluster.blog_id = selectedBlog.id
  cluster.assigned_blog_grade = selectedBlog.blog_grade  (이력 보존)
  cluster.intent_type = keyword.intentType               (Intent 기록 ★)
  cluster.intent_fit_score = 적합도 점수                   (성과 분석용 ★)
...
#### Intent × BlogGrade 프롬프트 매트릭스 ★

> 기존 BlogGrade만으로 프롬프트를 결정하면, S급+AD와 S급+INFO가 동일 전략을 쓰게 됨.
> Intent와 BlogGrade를 교차하여 **6×4 = 24가지 프롬프트 전략**을 정의.

```
■ S급 블로그

  S+AD:      전문가 추천 톤, adKeywords 고밀도, 가격 비교표 필수, CTA 강력
             독자: "지금 바로 가입/구매하려는 사람"
  S+COMPARE: 데이터 중심 비교, 표/차트 활용, 중립 어조 + 결론 명확
             독자: "A와 B 중 고민하는 구매 직전 사람"
  S+REVIEW:  심층 사용 후기, 장단점 솔직, E-E-A-T 경험 신호 강조
             독자: "실사용 후기가 궁금한 신중한 소비자"
  S+CRITIC:  전문적 분석, 근거 기반 비판, 대안 제시
             독자: "리스크를 파악하고 싶은 고관여 독자"

■ A급 블로그

  A+AD:      친근한 추천 톤, adKeywords 적정 밀도, 경험담 기반 자연스러운 유도
  A+REVIEW:  균형 잡힌 리뷰, 일상 경험 중심, 공감 요소 강화
  A+COMPARE: 실용적 비교, 가성비 중심, 표 활용
  A+INFO:    체계적 설명, 단계별 가이드, FAQ 풍부
  A+CRITIC:  우려 사항 정리, 주의점 나열, 대안 간략 제시

■ B급 블로그

  B+INFO:    쉬운 설명, 초보자 가이드, 검색량 높은 기본 정보
  B+TREND:   트렌드 요약, 시의성 강조, 빠른 발행 우선
  B+REVIEW:  간단 체험기, 일상적 톤, 접근성 높은 구성
  B+COMPARE: 간략 비교, 핵심 차이만, 짧고 명확

■ NEW 블로그

  NEW+INFO:  가장 기초적 설명, 검색 유입 극대화, 실적 씨앗 역할
...
interface AIGenerateInput {
  ...
  blogGrade: 'S' | 'A' | 'B' | 'NEW'
  intentType: IntentType          // AD/REVIEW/INFO/CRITIC/COMPARE/TREND ★ 추가
