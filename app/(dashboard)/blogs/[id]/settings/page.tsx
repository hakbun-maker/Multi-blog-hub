...
import { ArrowLeft, Save, Trash2, Pencil, X, CheckCircle2, XCircle, Loader2, RefreshCw, Clock } from 'lucide-react'
...
export default function BlogSettingsPage({ params }: { params: { id: string } }) {
  ...
  // 도메인 상태
  type DomainStatus = 'idle' | 'checking' | 'connected' | 'pending_dns' | 'not_registered' | 'error'
  const [domainStatus, setDomainStatus] = useState<DomainStatus>('idle')
  const [domainStatusMsg, setDomainStatusMsg] = useState('')
  const domainCheckTimer = useRef<ReturnType<typeof setInterval> | null>(null)

  const checkDomainStatus = useCallback(async (blogId: string) => {
    setDomainStatus('checking')
    try {
      const res = await fetch(`/api/blogs/${blogId}/domain`)
      ...
      if (data.status === 'none') {
        ...
        setDomainStatusMsg('도메인이 정상 연결되었습니다.')
        if (domainCheckTimer.current) clearInterval(domainCheckTimer.current)
      } else if (data.error === 'Vercel에 등록되지 않음') {
        setDomainStatus('not_registered')
        setDomainStatusMsg('Vercel에 아직 등록되지 않았습니다.')
      } else {
        setDomainStatus('pending_dns')
        setDomainStatusMsg('DNS 전파 중입니다. 최대 48시간 소요될 수 있습니다.')
      }
    } catch {
      setDomainStatus('error')
      setDomainStatusMsg('상태 확인 중 오류가 발생했습니다.')
    }
  }, [])

  const registerDomainToVercel = useCallback(async (blogId: string) => {
    setDomainStatus('checking')
    try {
      const res = await fetch(`/api/blogs/${blogId}/domain`, { method: 'POST' })
      const data = await res.json()
      if (!res.ok) {
        setDomainStatus('error')
        setDomainStatusMsg(data.error ?? 'Vercel 등록 실패')
        return
      }
      if (data.verified) {
        setDomainStatus('connected')
        ...
        setDomainStatus('pending_dns')
        setDomainStatusMsg('Vercel 등록 완료! DNS 전파 중입니다. 최대 48시간 소요됩니다.')
        // 폴링 시작
        if (domainCheckTimer.current) clearInterval(domainCheckTimer.current)
        let count = 0
        domainCheckTimer.current = setInterval(async () => {
          count++
          await checkDomainStatus(blogId)
          if (count >= 20) {
            if (domainCheckTimer.current) clearInterval(domainCheckTimer.current)
          }
        }, 30_000)
      ...
      setDomainStatus('error')
      setDomainStatusMsg('Vercel 등록 중 오류가 발생했습니다.')
    }
  }, [checkDomainStatus])
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // 도메인이 있으면 상태 초기 확인
      if (blogData.custom_domain) checkDomainStatus(params.id)
    }
    fetchData()
  }, [params.id, router, checkDomainStatus])
  ...
  const handleSaveBasic = async () => {
    ...
    // 커스텀 도메인이 있으면 상태 확인 폴링 시작 (30초마다, 최대 10분)
    if (customDomain) {
      if (domainCheckTimer.current) clearInterval(domainCheckTimer.current)
      // 저장 직후 즉시 1회 확인
      await checkDomainStatus(params.id)
      // 연결될 때까지 30초마다 자동 재확인
      let count = 0
      domainCheckTimer.current = setInterval(async () => {
        count++
        await checkDomainStatus(params.id)
        if (count >= 20) {  // 10분(20 * 30초) 후 자동 중단
          if (domainCheckTimer.current) clearInterval(domainCheckTimer.current)
        }
      }, 30_000)
    } else {
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <Input value={customDomain} onChange={e => { setCustomDomain(e.target.value); setDomainStatus('idle') }}
                placeholder="moneycopyway.ruhaworld.co.kr" className="flex-1" />
              {customDomain && blog?.custom_domain === customDomain && (
                <Button type="button" variant="outline" size="sm"
                  onClick={() => checkDomainStatus(params.id)}
            ...
            {domainStatus === 'pending_dns' && (
              <div className="flex items-center gap-1.5 text-xs text-amber-600 bg-amber-50 px-3 py-2 rounded-md">
                <Clock className="w-3.5 h-3.5 shrink-0 animate-pulse" />
                <span>{domainStatusMsg}</span>
              </div>
            )}
            {domainStatus === 'not_registered' && (
              <div className="flex items-center justify-between gap-2 text-xs text-red-600 bg-red-50 px-3 py-2 rounded-md">
                ...
                <Button
                  type="button"
                  size="sm"
                  className="h-6 text-xs px-2 bg-red-600 hover:bg-red-700 text-white shrink-0"
                  onClick={() => registerDomainToVercel(params.id)}>
                  지금 등록
                </Button>
              </div>
            )}
            ...
            <details className="text-xs text-gray-500 cursor-pointer">
              <summary className="hover:text-gray-700">도메인 연결 방법 (DNS 설정 가이드)</summary>
              <div className="mt-2 p-3 bg-gray-50 rounded-md space-y-1.5 text-gray-600">
                <p className="font-medium">서브도메인 (권장) — blog.example.com</p>
                <code className="block bg-white border rounded px-2 py-1">CNAME → cname.vercel-dns.com</code>
                <p className="font-medium mt-2">루트 도메인 — example.com</p>
                <code className="block bg-white border rounded px-2 py-1">A → 76.76.21.21</code>
                <p className="mt-1 text-gray-400">DNS 등록 후 저장하면 자동으로 Vercel에 연결됩니다. DNS 전파는 최대 48시간 소요됩니다.</p>
              </div>
            </details>
          </div>
